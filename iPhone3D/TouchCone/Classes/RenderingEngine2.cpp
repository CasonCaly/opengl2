...
void RenderingEngine2::Render() const
{
    ...
    mat4 modelviewMatrix =  scale*rotation;
    
    if(change)
    {
        float* p = (float*)&modelviewMatrix;
        for (int i = 0;  i < 4; i++) {
            std::cout<<*p<<" "<<*(p + 1) <<" "<<*(p+2)<<" "<<*(p+3)<<endl;
            p += 4;
        }
    ...
    modelviewMatrix = modelviewMatrix* translation;
